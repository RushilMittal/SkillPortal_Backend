 ...
 *  except the ones user has already rated
 ...
 public List<SubSkillDomain> getAllUnassignedSubSkills(String empId, String skillId){
        empId = empId.trim();
   ...
   //transforming subskills to subskill domain type
     ...
     for(SubSkill iterable: subSkills){
         ...
         SubSkillDomain temp = new SubSkillDomain(iterable.getId(),iterable.getName(),iterable.getSkillId(),getSubSkillCount(iterable.getId()));

         subSkillDomains.add(temp);
     }
     
     //getting list of employee skill rated by an employee
     List<EmployeeSkillDomain> skillEmp = getAllEmployeeSkillData(empId);
     
     List<SubSkillDomain> unassignedSkills = new LinkedList<>();
     
     /*iterating through subskill domain to check which subskills exist under
      * employeeskill , i.e, has been rated by employee and adding that 
      * from unassigned skills list
      */
     for(Iterator<SubSkillDomain> iter = subSkillDomains.iterator(); iter.hasNext();)
     {
     	SubSkillDomain i = iter.next();
     	boolean flag = false;
     	for (EmployeeSkillDomain j: skillEmp)
     	{
     		if(i.getId().equalsIgnoreCase(j.getSubSkill().getId()))
     		{
     	...
     	//adding subskill to subskillsdomain if flag is not set to true
        if(!flag)
        {
